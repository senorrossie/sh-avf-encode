...
int main(int argc, const char **argv) {
	...
	for(int i=0; i<15; ++i) {
		itab[i+1] = 40.0f * cosf(3.1415926535f * (float)i / 7.5f);
		qtab[i+1] = 40.0f * sinf(3.1415926535f * (float)i / 7.5f);
	}
		...
		for(int y=0; y<HEIGHT; ++y) {
			if (y & 1) {
				...
				for(int x=0; x<80; ++x) {
					...
					float rawhue = atan2f(fq, fi) * (7.5f / 3.1415926535f) + 2.0f;
					int ihue = (int)floorf(rawhue + 0.5f);

					int color = ((ihue + 15*1000) % 15) + 1;
					int out = 0;
